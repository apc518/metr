...
class MetricTreeDrawer{
    ...
    _draw(tree, depth, progress, isLowerTree, leafProgressValues, horizontalScale, horizontalOffset, index){
        ...
        if (depth <= this.totalMaxDepth - currentPatch.numLayersMuted && (!leaf || this.drawLeafNodes)){
            ...
            text(textValue, horizontalOffset + tree.pos.x, tree.pos.y + (leaf ? this.leafNodeTextSize : this.innerNodeTextSize) * 0.05);
        ...
        if (depth < this.totalMaxDepth - currentPatch.numLayersMuted){
            if (!leaf){
                ...
                for (let i = 0; i < tree.children.length; i++){
                    ...
                    line(horizontalOffset + tree.pos.x, yValues[0], horizontalOffset + t.pos.x, yValues[1]);
                }
